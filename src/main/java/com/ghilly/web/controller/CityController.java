...
public class CityController {
    ...
    //change url
    @GetMapping("/all/country/{countryId}")
    public ResponseEntity<List<CityDAO>> getCitiesByCountry(@PathVariable int countryId) {
        List<CityDAO> citiesByCountry = cityHandler.getCitiesByCountry(countryId);
        return ResponseEntity.ok().body(citiesByCountry);
    }

    @GetMapping("/all/country/{countryId}/capital")
    public ResponseEntity<CityDAO> getCapital(@PathVariable int countryId) {
        CityDAO capital = cityHandler.getCapital(countryId);
        return ResponseEntity.ok().body(capital);
    }
}