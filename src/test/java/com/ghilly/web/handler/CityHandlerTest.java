...
class CityHandlerTest {
    ...
    private static final String MOSCOW_LOWER_CASE = "moscow";
    private static final int COUNTRY_ID = 1;
    private static final int CITY_ID = 7;
    private static final CountryDAO RUS = new CountryDAO(COUNTRY_ID, "russia");
    private static final CityDAO MOS_DAO_FROM_REPO = new CityDAO(CITY_ID, MOSCOW_LOWER_CASE, RUS, true);
    private static final CityDAO MOS_DAO = new CityDAO(MOSCOW, RUS, true);
    private static final CityDAO MOS_DAO_FOR_SERVICE = new CityDAO(MOSCOW_LOWER_CASE, RUS, true);
    ...
    void createSuccess() {
        ...
        assertAll(
                ...
                () -> verify(cityServiceRest).theSameCityExists(COUNTRY_ID, MOSCOW_LOWER_CASE),
                () -> verify(countryServiceRest).getCapitalByCountryId(COUNTRY_ID),
                () -> verify(cityServiceRest).create(MOS_DAO_FOR_SERVICE),
    ...
    void createCityAlreadyExistsFail() {
        when(countryServiceRest.countryIdExists(COUNTRY_ID)).thenReturn(true);
        when(countryServiceRest.getCountryById(COUNTRY_ID)).thenReturn(RUS);
        when(cityServiceRest.theSameCityExists(COUNTRY_ID, MOSCOW_LOWER_CASE)).thenReturn(true);
        CityAlreadyExistsException exception = assertThrows(CityAlreadyExistsException.class,
                () -> handler.create(MOS_DAO, COUNTRY_ID));

        assertAll(

                () -> assertEquals("The city " + MOSCOW_LOWER_CASE + " already exists.",
                        exception.getMessage()),
                () -> verify(countryServiceRest).countryIdExists(COUNTRY_ID),
                () -> verify(cityServiceRest).theSameCityExists(COUNTRY_ID, MOSCOW_LOWER_CASE),
    ...
    void getAllCities() {
        ...
        List<CityDAO> cities = List.of(MOS_DAO_FOR_SERVICE, new CityDAO(sochi, RUS, false), new CityDAO(spb, RUS, false));
        ...
        assertAll(
                ...
                () -> assertEquals(actual.get(0).getName(), MOSCOW_LOWER_CASE),
    ...
    void updateSuccess() {
        ...
        assertAll(
                ...
                () -> verify(countryServiceRest).countryIdExists(COUNTRY_ID),
                () -> verify(countryServiceRest).getCountryById(COUNTRY_ID),
                () -> verify(cityServiceRest).theSameCityExists(COUNTRY_ID, newName.toLowerCase()),
                () -> verify(countryServiceRest).getCapitalByCountryId(COUNTRY_ID),
    ...
    void updateExistingCityFail() {
        ...
        when(cityServiceRest.cityIdExists(CITY_ID)).thenReturn(true);
        when(countryServiceRest.countryIdExists(COUNTRY_ID)).thenReturn(true);
        when(cityServiceRest.theSameCityExists(COUNTRY_ID, newName.toLowerCase())).thenReturn(true);
